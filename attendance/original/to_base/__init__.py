...
def get_viin_brand_resource_path(mod, *args):
    ...
    # exists in branding_module/static/src/img/favicon.ico
    if mod == 'web' and 'static/src/img/favicon.ico' in args:
        branding_module = _get_branding_module()
        for adp in odoo.addons.__path__:
...
def _update_favicon(env):
    ...
    for adp in odoo.addons.__path__:
        if viin_brand_manifest.get('installable', False):
            img_path = module.get_resource_path(branding_module, 'static/src/img/favicon.ico')
            if img_path:
